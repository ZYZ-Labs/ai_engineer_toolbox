...
import {
  ...
  readLectureExample,
  readLectureMarkdown,
  transformerLectureChapters
} from "@/lib/courses/transformer-lectures";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: lecture ? `Chapter ${lecture.number}: ${lecture.titleEn}` : "Transformer Lecture"
  };
}

export default async function TransformerLecturePage({ params }: Props) {
  const { stage, chapter } = await params;
  const lecture = findLectureChapter(stage, chapter);
  if (!lecture) notFound();
  ...
  const currentIndex = transformerLectureChapters.findIndex((item) => item.stage === stage && item.slug === chapter);
  const previous = transformerLectureChapters[currentIndex - 1];
  const next = transformerLectureChapters[currentIndex + 1];
        ...
        <aside className="space-y-4">
          <div className="rounded-spec border border-line bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-2 text-sm">
              {previous ? (
                <Link className="rounded-xl border border-line px-3 py-2 text-muted hover:border-primary/40 hover:text-primary" href={`/study/transformer-lectures/${previous.stage}/${previous.slug}`}>
                  Previous: Ch{previous.number}
                </Link>
              ) : null}
              {next ? (
                <Link className="rounded-xl border border-line px-3 py-2 text-muted hover:border-primary/40 hover:text-primary" href={`/study/transformer-lectures/${next.stage}/${next.slug}`}>
                  Next: Ch{next.number}
                </Link>
              ) : null}
            </div>
